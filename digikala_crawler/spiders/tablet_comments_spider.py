...
class TabletCommentsSpider(scrapy.Spider):
    name = 'tablet_comments_spider'
    i = 0
    # sort by most visited by default
    start_urls = ['https://www.digikala.com/search/category-tablet/?sortby=4']
    ...
    def parse(self, response):
        ...
        # next_page_url = 'https://www.digikala.com/search/category-mobile-phone/?pageno={}&sortby=4'.format(next_page)
        if next_page < 50 and next_page_url is not None:
    ...
    def parse_product(self, response):
        ...
        if len(title_splitted) == 2:
            ...
            model = title_splitted[0].split('تبلت')[1]
    ...
    def parse_comments(self, response):
        ...
        for comment in response.css('ul.c-comments__list li section'):
            ...
            item['comment_body'] = comment.css('div.article p::text').get().strip()
        ...
        next_page_url = response.css('.c-pager ul.c-pager__items li a.is-active') \
            .xpath('../following-sibling::li[1]//a/@href').extract_first()
        if next_page < 1000 and next_page_url is not None:
